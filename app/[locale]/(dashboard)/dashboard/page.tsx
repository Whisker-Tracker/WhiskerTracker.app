...
export default async function DashboardPage({
  ...
  const logsKpi = await getTranslations("logsKpi");
  ...
  const tnrSegments = [
    { key: "unaltered", label: catsT("tnrStatusUnaltered"), value: unalteredCount, color: "#64748b" },
    { key: "trapped", label: catsT("tnrStatusTrapped"), value: trappedCount, color: "#f59e0b" },
    { key: "neutered_spayed", label: catsT("tnrStatusNeutered"), value: neuteredCount, color: "#06b6d4" },
    { key: "eartipped", label: catsT("tnrStatusEartipped"), value: eartippedCount, color: "#0d9488" },
  ];
  ...
  for (const log of logs) {
    const colonyName = log.colonies?.name ?? t("unnamedColony");
  ...
  return (
    <div className="space-y-4">
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          label={t("coloniesCount")}
          value={String(colonyCount ?? 0)}
          hint={t("mappedTerritories")}
          className="!border-rose-200 !bg-rose-50"
        />
        <MetricCard
          label={t("catsCount")}
          value={String(catCount ?? 0)}
          hint={t("acrossAllColonies")}
          className="!border-sky-200 !bg-sky-50"
        />
        <MetricCard
          label={t("eartippedCount")}
          value={`${tnrPct}%`}
          hint={`${eartippedCount} ${t("eartippedPercentage")}`}
          className="!border-amber-200 !bg-amber-50"
        />
        <MetricCard
          label={t("adoptedCats")}
          value={String(adoptedCatsCount ?? 0)}
          hint={t("adoptedCatsHint")}
          className="!border-emerald-200 !bg-emerald-50"
        />
        <TnrDonutCard
          title={t("tnrDonutTitle")}
          hint={t("tnrDonutHint")}
          centerLabel={t("cats")}
          total={catCount ?? 0}
          ...
          {topColony ? (
            <div className="mt-3 space-y-3">
              ...
              <div className="space-y-2">
                ...
                {colonyHours.length === 0 ? <p className="text-sm text-slate-500">{t("noData")}</p> : null}
              </div>
            </div>
          ) : (
